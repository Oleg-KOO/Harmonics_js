...
class LineFunc{
	constructor(point1, point2, scale, htmlItem, namberPoint, shift){
		this.htmlItem = htmlItem
		this.point1 = point1;
		this.point2 = point2;
		this.scale = scale;
		this.namberPoint = namberPoint
		this.shift = shift;
		this.dependentElements = [];
		this.ratioList = new Map();
		this.text = '';
		this.SetParam();
		point1.SetLine([this, this.Refresh1]);
		point2.SetLine([this, this.Refresh2]);
	}
	...
	SetParam(){
		...
		if (this.a != null && this.zeroX > this.minX && this.zeroX < this.maxX) this.zeroCross = true;
		else this.zeroCross = false;
		this.zeroCrossY = (this.minX * this.maxX) < 0;
		this.ratioList.clear();
		let text1 = this.a == 0 ? '': '·';
		if (this.minX == this.maxX) this.text = '';
	...
	Refresh1(x, y){
		if (this.namberPoint == 0) x = -x;
		this.htmlItem.SetAttr({'x1': this.scale.X(x + this.shift), 'y1': this.scale.Y(y)});
		this.SetParam();
		for (let elem of this.dependentElements) elem[1].call(elem[0], this);
	}
	...
	GetIntegral(countPoint, period, harmonic){
		...
		function GetInterval(){
			...
			if (zeroCross && zeroCrossY && zeroX != 0){
			...
			if (zeroCross) return [[minX, zeroX, GetPeriod(minX, zeroX)], [zeroX, maxX, GetPeriod(zeroX, maxX)]];
...
class LineGraph{
	...
	GetHarmonic(countPoint, period, harmonic){
		let ka = 0; let kb = 0;
		for (let line of this.lineList){
			let koef = line.GetIntegral(countPoint, period, harmonic);
			ka += koef.ka;  kb += koef.kb;
		}
...
class Harmonic{
	constructor(lineGraph, countPoint, period, harmonicNumber){
		this.namber = harmonicNumber;
		this.lineGraph = lineGraph;
		this.countPoint = countPoint;
		this.period = period;
		this.Refresh();
		this.use = true;
		if (this.ka == 0 && this.kb == 0) this.use = false;
	}
	
	y(x){
		if (this.namber == 0) return this.ka / this.period;
		return this.ka * Math.cos(Math.PI * this.namber * x / this.period * 2)/ this.period * 2 +
			   this.kb * Math.sin(Math.PI * this.namber * x / this.period * 2)/ this.period * 2;
	}
	
	Refresh(){
		let koef = this.lineGraph.GetHarmonic(this.countPoint, this.period, this.namber);
		this.ka = koef.ka;
		this.kb = koef.kb;
		this.text = '';
		let textKoef = this.kb < 0 ? '': '+';
		if (this.ka != 0) this.text = this.text + this.ka.toPrecision(3) + '·cos(' + this.namber + '·π·x/' + (this.period / 2) + ')';
		if (this.kb != 0) this.text = this.text + textKoef + this.kb.toPrecision(3) + '·sin(' + this.namber + '·π·x/' + this.period / 2 + ')';
		if (this.namber == 0) this.text = this.ka.toPrecision(3);
	}
...
class HarmonicGraph{
	...
	constructor(field, harmonicsField, lineGraph, gridData, scale, countHarmonic, toRefresh){
		this.lineGraph = lineGraph;
		this.listHarmonic = [];
		...
		this.harmonic = field.AddSVG({
				'tag': 'polyline', 
				});
		for (let i = 0; i <= countHarmonic; i++) this.AddHarmonic();
	}
	...
	GetHarmonic(namber){
		...
		let points = '';
		let item;
		for (let newItem of this.listHarmonic) if (newItem.namber == namber) {item = newItem; break;}
		for (let x = this.gridData.minX; x <= this.gridData.maxX; x += step){
	...
	AddHarmonicDisplay(namber){
		let newHarmonic = this.GetHarmonic(namber);
		let color = colorList[namber % colorList.length];
		let htmlItem = this.harmonicsField.AddSVG({
			'tag': 'polyline',
			'stroke': color,
			'points': newHarmonic
			});
		this.harmonicsDisplay.set(namber, htmlItem);
	}
	
	RemoveHarmonicDisplay(namber){
		if (!this.harmonicsDisplay.has(namber)) return;
		this.harmonicsDisplay.get(namber).parentElement.removeChild(this.harmonicsDisplay.get(namber));
		this.harmonicsDisplay.delete(namber);
	}
