...
let divBox = svgBox.AddSVG({
	...
	'width': '100%', 
	'height': '100%',
	'viewBox': '0 0 ' + fieldData.svgWidth + ' ' + fieldData.svgHeight
	});
...
defs.appendChild(GetShadowFilter('shadowPoints', scale.X(fieldData.minX), scale.Y(fieldData.maxY), 5, 5, 5));
defs.appendChild(GetShadowFilter('shadowHarmonics', scale.X(fieldData.minX), scale.Y(fieldData.maxY), 3, 3, 4));
defs.appendChild(GetShadowFilter('shadowLineGraf', scale.X(fieldData.minX), scale.Y(fieldData.maxY), 5, 5, 5));
defs.appendChild(GetShadowFilter('shadowHarmonicGraf', scale.X(fieldData.minX), scale.Y(fieldData.maxY), 7, 7, 7));	

harmonics.SetAttr(harmonicsFormat);
lineFunc.SetAttr(lineFuncFormat);
harmonicFunc.SetAttr(harmonicFuncFormat);
CreateGridLine(grid, fieldData, majorDivisionsFormat, auxiliaryDivisionsFormat);
CreateAxisLine(grid, fieldData, axisFormat, textFormat);	
...
let points = new PointList(pointsField, fieldData, divBox);
let lineGraph = new LineGraph (lineFunc, points.listPoint, fieldData);
let iformBlock = new InformationBlock(lineFunkText, harmonicsText, Number(harmonicCount.innerHTML));
let harmonicGraph = new HarmonicGraph(harmonicFunc, harmonics, lineGraph, fieldData, Number(harmonicCount.innerHTML));
